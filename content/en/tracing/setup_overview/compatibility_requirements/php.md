...
We do not offer support for tracing processes forked using [pcntl][6]. When a call to `pcntl_fork` is detected, we disable tracing in the forked process. The main process is still be traced. To request full support for `pcntl`, contact our [support team][2].
